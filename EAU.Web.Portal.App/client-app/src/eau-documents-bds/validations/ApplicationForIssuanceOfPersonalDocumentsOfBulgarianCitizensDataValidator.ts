...
import { DocumentFormValidationContext, ErrMsgCodesConstants, IdentityDocumentType } from "eau-documents";
import { ApplicationForIssuanceOfPersonalDocumentsOfBulgarianCitizensDataVM } from "../models/ModelsAutoGenerated";
import { PersonDataValidator } from "./PersonDataValidator";
...
export class ApplicationForIssuanceOfPersonalDocumentsOfBulgarianCitizensDataValidator
    ...
    constructor() {
        ...
        this.ruleFor(m => m.person).setValidator(new PersonDataValidator()).when(x => x.identificationDocuments && x.identificationDocuments.length > 0 && x.identificationDocuments[0] != IdentityDocumentType.DrivingLicense);

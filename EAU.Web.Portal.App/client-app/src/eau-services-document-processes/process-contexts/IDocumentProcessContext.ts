...
import { DocumentProcess, ProcessStatuses } from '../models/DocumentProcess';
...
export class DocumentProcessContextBase implements IDocumentProcessContext {
    ...
    saveDocumentForm(): Promise<void> {
        ...
        if (this.docFormLastSavedState != docFormState) {
            this.processDataService.saveDocumentForm(this.process.documentProcessID, this.formManager.documentForm).bind(this).then(() => {
    ...
    deleteDocumentProcess(): Promise<void> {
        return this.processDataService.deleteDocumentProcess(this.process.documentProcessID).bind(this).then(() => {
            this.clearContext();
        })
    }

    startSigning(): Promise<void> {
        return this.processDataService.startSigning(this.process.documentProcessID).bind(this).then(signGuid => {
            runInAction(() => {
    ...
    startSending(): Promise<void> {
        return this.processDataService.startSending(this.process.documentProcessID).bind(this).then(() => {
    ...
    protected initDocumentProcessContext(process: DocumentProcess): Promise<void> {
        ...
        var srvPromis = Nomenclatures.getServices(s => s.serviceID == process.serviceID);

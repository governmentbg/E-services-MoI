import { BaseDataModel, ErrorInfo, ObjectHelper, UserInfo } from 'cnsys-core';
...
import { ProcessStatuses } from '../models/DocumentProcess';
...
@observer class DocumentProcessUIImpl extends EAUBaseComponent<DocumentProcessUIProps, any> {
    ...
    renderInProcess() {
        ...
        return (
            <>
                {
                    ObjectHelper.isSubClassOf(this.props.context.formManager, ApplicationFormManagerBase)
                        && ((this.props.context.formManager.documentForm as ApplicationFormVMBase).authorHasNonHandedSlip
                            || (this.props.context.formManager.additionalData.showMessageForNonPaidSlip
                                && this.props.context.formManager.additionalData.showMessageForNonPaidSlip.toString() == "true")
                            || ((this.props.context.formManager.documentForm as ApplicationFormVMBase).authorHasNonPaidSlip
                                && this.props.context.formManager.additionalData.showWarningForNonPaidSlip
                                && this.props.context.formManager.additionalData.showWarningForNonPaidSlip === true)
                        )
                        ? <div className="general-message-wrapper">
                            {
                                (this.props.context.formManager.documentForm as ApplicationFormVMBase).authorHasNonHandedSlip
                                && <div className="alert alert-warning" role="alert"> <p>{this.getResource('GL_ANDOBLIGATION_ACT_I')}</p></div>
                            }
                            {
                                this.props.context.formManager.additionalData.showMessageForNonPaidSlip
                                && this.props.context.formManager.additionalData.showMessageForNonPaidSlip.toString() == "true"
                                && <div className="alert alert-info" role="alert"> <p>{this.getResource('GL_00028_I')}</p></div>
                            }
                            {
                                (this.props.context.formManager.documentForm as ApplicationFormVMBase).authorHasNonPaidSlip
                                && this.props.context.formManager.additionalData.showWarningForNonPaidSlip
    ...
    renderButtonBar() {
        return (
            <div className="button-bar button-bar--form button-bar--responsive">
                ...
                <div className="left-side">
                    {((this.props.context.formManager.mode != DocumentModes.ViewDocument) || this.props.showRejectButtonInsteadOfNewDocButton) ?
                        <ConfirmationModal modalTitleKey='GL_REJECT_APPLICATION_L' modalTextKeys={deleteDocProcessMsgKeys} onSuccess={this.delete} yesTextKey='GL_OK_L' noTextKey='GL_NO_L'>
                            <Button type="button" className="btn btn-secondary" lableTextKey="GL_REJECT_L"></Button>
                        </ConfirmationModal>
                        :
    ...
    changeSection(code: string) {
        ...
        if (this.currentSection) {
            ...
            if (isValid || code != this.currentSection.code && this.compareErrors(tmpCurrentErrors, this.currentSection.errors)) {
                this.props.context.saveDocumentForm();
    ...
    delete() {
        this.props.registerAsyncOperation(this.props.context.deleteDocumentProcess().bind(this).then(() => {
    ...
    saveDocumentPeriodically() {
        ...
        that.saveDocTimeout = setTimeout(() => {
            that.props.context.saveDocumentForm().then(() => {
                that.saveDocumentPeriodically()
            })

import { UrlHelper, UserInfo, ObjectHelper } from 'cnsys-core';
...
@observer class ApplicationProcessUIImpl extends EAUBaseComponent<ApplicationProcessUIProps, any> {
    ...
    constructor(props?: ApplicationProcessUIProps) {
        ...
        this.props.registerAsyncOperation(eauAuthenticationService.getCurrentUser().bind(this).then(usr => {
            if (usr) {
                ...
                return this.processContext.tryLoadApplicationProcess(this.props.match.params.serviceID).bind(this).then(isLoaded => {
                    if (!isLoaded) {
                        if (this.instructionURI) {
                            return this.processContext.createApplicationProcess(this.props.match.params.serviceID, this.instructionURI, null, this.caseFileURI);
                        }

                        if (this.additionalApplicationURI) {
                            return this.processContext.createApplicationProcess(this.props.match.params.serviceID, null, this.additionalApplicationURI, this.caseFileURI);
                        }
                    }
                    else {
                        if ((this.instructionURI || this.additionalApplicationURI) &&
                            ...
                            return this.processContext.deleteDocumentProcess().bind(this).then(() => {
                                return this.processContext.createApplicationProcess(this.props.match.params.serviceID, this.instructionURI, this.additionalApplicationURI, this.caseFileURI);
                            })
                        }
                    }
                });
    ...
    renderProcessExistsInfo(processStatus: ProcessStatuses) {
        ...
        switch (processStatus) {
            ...
            case ProcessStatuses.Signing:
                return <Alert color="warning">{this.getResource("GL_00004_E")}</Alert>
    ...
    onProcessDeleted() {
        this.props.routerExt.goTo(Constants.PATHS.APPLICATION_PROCESSES.replace(':serviceID', this.service.serviceID.toString()).replace('/:sectionCode?', ''), null);

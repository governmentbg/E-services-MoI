...
export class NotificationForTakingOrRemovingFromSecurityDataValidator
    ...
    constructor() {
        ...
        this.ruleFor(m => m.securityContractData).notEmpty()
            .withMessage(ResourceHelpers.formatErrorMessage(ErrMsgCodesConstants.DefaultNotEmptyErrorMessage, new NotificationForTakingOrRemovingFromSecurityDataVM(), 'securityContractData'));

        this.ruleFor(m => m.securityContractData).setValidator(new SecurityContractDataValidator());

        this.ruleFor(m => m.contractAssignor).setValidator(new ContractAssignorValidator()).when(m => m.notificationType == NotificationType.NewSecurityContr235789 || m.notificationType == NotificationType.TerminationSecurityContr235789);
    }
}

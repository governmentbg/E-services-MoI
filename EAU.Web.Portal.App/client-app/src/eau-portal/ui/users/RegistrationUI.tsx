                        ...
                        <fieldset className="fields-group">
                            ...
                            <div className="row mt-2">
                                <div className="form-group col-sm order-0 order-sm-2">
                                    <button className="btn btn-primary btn-block" aria-label="Вход в системата" type="button" onClick={this.register}>{this.getResource("GL_REGISTER_L")}</button>
                                </div>
            ...
            this.props.registerAsyncOperation(new UserService().register(this.model).then(result => {
                runInAction(() => {
                    ...
                    if (result && result.emailAlreadyExists && result.emailUserStillNotActivated) {
                        ...
                        this.notification = <Alert color="success">{resourceManager.getResourceByKey("GL_00012_I")} <button className="btn btn-link" onClick={this.resendConfirmationEmail}>{this.getResource("GL_USR_SEND_REG_L")}</button></Alert>
                    }

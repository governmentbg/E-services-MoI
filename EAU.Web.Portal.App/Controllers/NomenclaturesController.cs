...
namespace EAU.Web.Portal.App.Controllers
{
    ...
    public class NomenclaturesController : BaseApiController
    {
        ...
        /// <returns>Номенклатура за етикети.</returns>
        [HttpGet]
        [Route("DeliveryUnitsInfo/{serviceID}")]
        public async Task<IActionResult> SearchDeliveryUnitsInfo(int serviceID, CancellationToken token, [FromServices] IDeliveryUnitsInfo deliveryUnitsInfo)
        {
            await deliveryUnitsInfo.EnsureLoadedAsync(serviceID, token);
            var cachedResources = deliveryUnitsInfo.Search(serviceID, out DateTime? lastModifiedDate);

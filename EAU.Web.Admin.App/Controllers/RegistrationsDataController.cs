...
namespace EAU.Web.Admin.App.Controllers
{
    ...
    public class RegistrationsDataController : BaseApiController
    {
        ...
        {
            var res = await registrationDataService.UpdateAsync(registrationDataID, item.Description, item.Cin, item.Email, item.SecretWord, item.ValidityPeriod, item.PortalUrl, item.NotificationUrl, item.ServiceUrl, cancellationToken);

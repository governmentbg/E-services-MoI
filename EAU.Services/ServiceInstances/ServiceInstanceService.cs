...
namespace EAU.Services.ServiceInstances
{
    ...
    public class ServiceInstanceService : IServiceInstanceService
    {
        ...
        private async Task<AdditionalData> GetAdditionalDataAsync(ServiceInstanceInfo serviceInstance, CancellationToken cancellationToken)
        {
            ...
            additionalData["currentStage"] = serviceInstance.UnexecutedStages.First().Name;
            additionalData["currentStageActualCompletionDate"] = serviceInstance.UnexecutedStages.First().ActualCompletionDate?.ToString("dd.MM.yyyy");
            ...
            if (lastDocument != null && lastDocument.DocumentTypeURI == DocumentTypeUrisKAT.DataForPrintSRMPS)
            {

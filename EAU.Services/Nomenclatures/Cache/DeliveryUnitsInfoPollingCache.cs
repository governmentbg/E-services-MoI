...
{
    public class DeliveryUnitsInfoPollingCache : DataCacheItems<CachedDataInfo<List<UnitInfo>>, int>, IDeliveryUnitsInfoCache
    {
        ...
        protected override IDataCacheItem<CachedDataInfo<List<UnitInfo>>> CreateCacheItem(int key)
        {
            return new PollingDataCacheItem<List<UnitInfo>>(_logger, _pollingCacheInfrastructure, async (etag, cancellationToken) =>
            {
                ...
                {
                    var data = await _waisIntegrationServiceClientsFactory.GetNomenclaturesClient().GetServiceDeliveryUnitsAsync(currentService.SunauServiceUri, CancellationToken.None);

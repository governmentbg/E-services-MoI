...
namespace EAU.KAT.Documents.Domain.Validations
{
    public class EntityDataValidator : EAUValidator<EntityData>
    {
        public EntityDataValidator()
        {
            ...
            RuleSet("Farmer", () =>
            {
                EAURuleFor(m => m.Identifier).RequiredField().EgnValidation().WithEAUErrorCode("GL_INVALID_BULSTAT_E"); ;
            });

            RuleSet("EntityOrFarmer", () =>
            {
                //EAURuleFor(m => m.Name).RequiredField();
                EAURuleFor(m => m.Identifier).RequiredField().EgnUICBulstatClientValidation().WithEAUErrorCode("GL_INVALID_IDENT_E"); ;
            });

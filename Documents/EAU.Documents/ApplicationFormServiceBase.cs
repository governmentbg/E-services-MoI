...
namespace EAU.Documents
{
    public class ApplicationFormInitializationRequest : DocumentFormInitializationRequest
    {
        ...
        public ObligationDocumentsByLicenceNumResponse ANDObligationDocuments { get; set; }
        ...
        where TView : ApplicationFormVMBase
    {
        ...
        protected override List<(byte[] Content, string Description, Guid Guid, string MimeType, string FileName, int? DocumentTypeID)> GetAttachedDocumentsInternal(TDomain formDoamin)
        {
            if (formDoamin.AttachedDocuments != null && formDoamin.AttachedDocuments.Count > 0)
            {
                ...
                {
                    result.Add((attDoc.FileContent, attDoc.Description, Guid.Parse(attDoc.UniqueIdentifier), attDoc.FileType, attDoc.FileName, null));
                }
        ...
        protected async override Task<OperationResult> InitializeDocumentFormInternalAsync(DocumentFormInitializationRequest request, CancellationToken cancellationToken)
        {
            ...
            if (request.Mode != DocumentModes.SignDocument && request.Mode != DocumentModes.ViewDocument)
            {
                ...
                {
                    var andResponse = await GetService<IANDServicesClientFactory>().GetANDServicesClient().GetObligationDocumentsByLicenceNumAsync(new ObligationDocumentsByLicenceNumRequest()
                    {
                    ...
                    {
                        initRequest.ANDObligationDocuments = andResponse.Response;
                    }
                }
        ...
        protected async virtual Task<OperationResult> InitializeApplicationFormInternalAsync(ApplicationFormInitializationRequest request, CancellationToken cancellationToken, bool skipBGDocumentValidation = false, bool skipValidation = false)
        {
            ...
            if (!isRemovingIrregularitiesInstruction && !request.AdditionalData.ContainsKey("additionalApplicationURI"))
            {
        ...
        protected async virtual Task<OperationResult> InitializeApplicationFormInternalAsync(ApplicationFormInitializationRequest request, CancellationToken cancellationToken)
        {
            return await InitializeApplicationFormInternalAsync(request, cancellationToken, false);
        }
        ...
        protected async virtual Task<IErrorCollection> ValidateApplicationFormInternalAsync(ApplicationFormValidationRequest request, CancellationToken cancellationToken, bool skipBGDocumentValidation = false, bool skipRecipientValidation = false)
        {
        ...
        protected async virtual Task<IErrorCollection> ValidateApplicationFormInternalAsync(ApplicationFormValidationRequest request, CancellationToken cancellationToken)
        {
            return await ValidateApplicationFormInternalAsync(request, cancellationToken, false, false);
        }
        ...
        public async Task CheckForNonHandedAndNonPaidSlip(ApplicationFormInitializationRequest request, TView app)
        {
            try
            {
                var obligatedPersonIdent = "";

                if (request.ApplicantInfo.PersonData.PersonIdentification.PersonIdentificationBG != null)
                {
                    obligatedPersonIdent = request.ApplicantInfo.PersonData.PersonIdentification.PersonIdentificationBG.PIN;
                }
                else if (request.ApplicantInfo.PersonData.PersonIdentification.PersonIdentificationF != null)
                {
                    obligatedPersonIdent = !string.IsNullOrWhiteSpace(request.ApplicantInfo.PersonData.PersonIdentification.PersonIdentificationF.PIN)
                        ? request.ApplicantInfo.PersonData.PersonIdentification.PersonIdentificationF.PIN
                        : request.ApplicantInfo.PersonData.PersonIdentification.PersonIdentificationF.LNC;
                }

                if (!string.IsNullOrEmpty(obligatedPersonIdent))
                {
                    var hasNonHandedSlipRes = await GetService<IANDServicesClientFactory>().GetANDServicesClient().CheckNonHandedSlipAsync(obligatedPersonIdent, CancellationToken.None);

                    if (hasNonHandedSlipRes.IsSuccessfullyCompleted)
                        app.AuthorHasNonHandedSlip = hasNonHandedSlipRes.Response;
                }
            }
            catch (Exception ex) { }

            if (request.ANDObligationDocuments != null)
            {
                ...
                app.AuthorHasNonPaidSlip = request.ANDObligationDocuments.allObligations != null && request.ANDObligationDocuments.allObligations.Length > 0;
            }
        }

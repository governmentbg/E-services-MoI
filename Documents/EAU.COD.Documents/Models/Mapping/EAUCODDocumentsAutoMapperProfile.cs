...
namespace EAU.COD.Documents.Models.Mapping
{
    public class EAUCODDocumentsAutoMapperProfile : Profile
    {
        ...
        private void CreateViewToDomainModelMap()
        {
            ...
            CreateMap<List<TerritorialScopeOfServicesDistrictsVM>, List<TerritorialScopeOfServicesDistricts>>()
               .AfterMap((s, d) =>
               {
                   foreach (var id in s)
                   {
                       TerritorialScopeOfServicesDistricts dest = new TerritorialScopeOfServicesDistricts()
                       {
                           DistrictGRAOCode = id.DistrictGRAOCode,
                           DistrictGRAOName = id.DistrictGRAOName,
                       };
            ...
            CreateMap<List<SecurityServiceTypesVM>, List<RequestForIssuingLicenseForPrivateSecurityServicesDataSecurityServiceTypes>>()
                .AfterMap((s, d, ctx) =>
                {
                    foreach (var id in s)
                    {
                        ...
                        {
                            var mappedId = id;
                            d.Add(ctx.Mapper.Map<SecurityServiceTypesVM, RequestForIssuingLicenseForPrivateSecurityServicesDataSecurityServiceTypes>(mappedId));
                        }
                    }
                });
            ...
            CreateMap<RequestForIssuingLicenseForPrivateSecurityServicesDataVM, RequestForIssuingLicenseForPrivateSecurityServicesData>()
                .AfterMap((s, d, ctx) =>
                {
                    d.SecurityServiceTypes = ctx.Mapper.Map<List<SecurityServiceTypesVM>, List<RequestForIssuingLicenseForPrivateSecurityServicesDataSecurityServiceTypes>>(s.SecurityServiceTypes);
                });

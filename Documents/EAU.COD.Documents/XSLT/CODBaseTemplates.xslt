...
<xsl:stylesheet version="1.0"
          ...
          <td colspan="2">
            ...
            <xsl:if test="$ContractAssignor/ca:AssignorPersonEntityType/. = 1">
              ...
              <xsl:call-template name="PersonAssignorData">
                ...
                <xsl:with-param name="IsGuardedPerson" select = "0" />
              </xsl:call-template>
            </xsl:if>
  ...
  <xsl:template name="PersonAssignorData">
    ...
    <xsl:param name="IsGuardedPerson"/>
    <xsl:choose>
      <xsl:when test = "$PersonAssignorData">
        <xsl:value-of select="$PersonAssignorData/pad:FullName" />,
        ЕГН/ЛН/ЛНЧ/ДДММГГГ&#160;  <xsl:value-of select="$PersonAssignorData/pad:Identifier" />
        <xsl:if test="$IsGuardedPerson = 1">
          &#160;, охранявано лице&#160;
          <xsl:if test="$PersonAssignorData/pad:GuardedType/. = '1'">
            Мъж
          </xsl:if>
          <xsl:if test="$PersonAssignorData/pad:GuardedType/. = '2'">
            Жена
          </xsl:if>
          <xsl:if test="$PersonAssignorData/pad:GuardedType/. = '3'">
            Момче
          </xsl:if>
          <xsl:if test="$PersonAssignorData/pad:GuardedType/. = '4'">
            Момиче
          </xsl:if>
        </xsl:if>
      </xsl:when>
    ...
    <xsl:choose>
      <xsl:when test = "$ProtectionPersonsProperty">        
        <xsl:choose>
          ...
          <xsl:otherwise>
            Наименование на охранявания обект:&#160;<xsl:value-of select="$ProtectionPersonsProperty/ppp:SecurityObjectName" />,<br/>
            Адрес на охранявания обект:&#160;<xsl:value-of select="$ProtectionPersonsProperty/ppp:Address" /><br/><br/>
      ...
      <xsl:when test = "$PersonalSecurity">
        <xsl:choose>
          <xsl:when test = "$ForTermination = 0">
            ...
            <xsl:call-template name="PersonAssignorData">
              <xsl:with-param name="PersonAssignorData" select = "$PersonalSecurity/ps:GuardedPerson" />
              <xsl:with-param name="IsGuardedPerson" select = "1" />
            ...
            , адрес на охраняваното лице&#160;<xsl:value-of select="$PersonalSecurity/ps:Address" />
            , възложителят е&#160;
    ...
    <xsl:choose>
      <xsl:when test = "$AlarmAndSecurityActivity">
        <xsl:choose>
          ...
          <xsl:otherwise>
            Наименование на охранявания обект:&#160;<xsl:value-of select="$AlarmAndSecurityActivity/aasa:SecurityObjectName" />,<br/>
            Адрес на охранявания обект:&#160;<xsl:value-of select="$AlarmAndSecurityActivity/aasa:Address" /><br/><br/>
  ...
  <xsl:template name="SecurityTransportingCargo">
    ...
    <xsl:param name="ForTermination"/>
    <xsl:choose>
      <xsl:when test = "$SecurityTransportingCargo">
        ...
        Териториален обхват на транспортирането до&#160;<xsl:value-of select="$SecurityTransportingCargo/stc:TerritorialScopeTo" /><br/>
        <br/>
        <xsl:choose>
          <xsl:when test = "$ForTermination = 0">
            ОРЪЖИЕ<br/>

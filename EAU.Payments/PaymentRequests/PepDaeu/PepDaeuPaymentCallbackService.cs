...
namespace EAU.Payments.PaymentRequests.PepDaeu
{
    ...
    public class PepDaeuPaymentCallbackService : IPepDaeuPaymentCallbackService
    {
        ...
        public async Task<OperationResult<string>> ProcessNotificationMessageAsync(string registrationDataCin, NotificationMessageTransportRequest message, CancellationToken cancellationToken)
        {
            ...
            catch (Exception ex)
            {
                ...
                response = EAUJsonSerializer.Serialize(new NotificationMessageResponse() { Success = false });
            }
            finally
            {
            ...
            return new OperationResult<string>(OperationResultTypes.SuccessfullyCompleted) { Result = response };
        }
        ...
        private async Task<NotificationMessageResponse> ProcessMessageAsync(NotificationMessageRequest notificationMessageRequest, CancellationToken cancellationToken)
        {
            var result = await _dbContextOperationExecutor.ExecuteAsync((Func<IDbContext, CancellationToken, Task<OperationResult<NotificationMessageResponse>>>)(async (dbContext, innerToken) =>
            {
                ...
                try
                {
                    var paymentRequestForLock = (await _paymentRequestService.SearchAsync(new PaymentRequestSearchCriteria() { ExternalPortalNumber = notificationMessageRequest.Id }, cancellationToken)).SingleOrDefault();
                    if (paymentRequestForLock == null)
                    {
                    ...
                    var paymentRequest = (await _paymentRequestService.SearchAsync(new PaymentRequestSearchCriteria() { ExternalPortalNumber = notificationMessageRequest.Id }, cancellationToken)).SingleOrDefault();
                    if (paymentRequest == null)
                    {
                        throw new NotSupportedException(string.Format("No payment found for externalPortalNumber №{0}", notificationMessageRequest.Id));
                    }
                        ...
                        || paymentRequest.Status == PaymentRequestStatuses.Expired))
                    {
                        ...
                        else if (paymentRequest.Status == PaymentRequestStatuses.Cancelled)
                        {
                            if (paymentRequest.AdditionalData == null)
                            {
                                paymentRequest.AdditionalData = new Utilities.AdditionalData();
                        ...
                        else if (paymentRequest.Status == PaymentRequestStatuses.Expired)
                        {
                            if (paymentRequest.AdditionalData == null)
                            {
                                paymentRequest.AdditionalData = new Utilities.AdditionalData();

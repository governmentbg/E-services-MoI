...
namespace EAU.Payments.PaymentRequests
{
    ...
    public class PaymentRequestService : IPaymentRequestService
    {
        ...
        public async Task<OperationResult<Models.PaymentRequest>> PaymentCancelled(Models.PaymentRequest request, CancellationToken cancellationToken)
        {
            return await PaymentChangeStatus(request, PaymentRequestStatuses.Cancelled, cancellationToken);
        }

        public async Task<OperationResult<Models.PaymentRequest>> PaymentExpired(Models.PaymentRequest request, CancellationToken cancellationToken)
        {
            return await PaymentChangeStatus(request, PaymentRequestStatuses.Expired, cancellationToken);
        }
        ...
        public async Task<OperationResult<Models.PaymentRequest>> StartPaymentAsync(long obligaionID, StartPaymentRequest request, CancellationToken cancellationToken)
        {
            var res = await _dbContextOperationExecutor.ExecuteAsync((Func<IDbContext, CancellationToken, Task<OperationResult<Models.PaymentRequest>>>)(async (dbContext, innerToken) =>
            {
                ...
                //За Пеп на ДАЕУ може да има само едно плащане и затова го изчитаме.
                if (request.RegistrationDataType.Value == RegistrationDataTypes.PepOfDaeu)
                {
                    var createdPaymentRequests = (await _paymentRequestRepository.SearchAsync(new PaymentRequestSearchCriteria()
                    {
                        ObligationIDs = new List<long>() { obligaionID },
                        RegistrationDataID = registrationData.RegistrationDataID
                    }, cancellationToken)).ToList();

                    if (createdPaymentRequests != null && createdPaymentRequests.Any())
                    {
                        return new OperationResult<Models.PaymentRequest>(OperationResultTypes.SuccessfullyCompleted) { Result = createdPaymentRequests.Single() };
                    }
                }
                ...
                var paymentRequest = new Models.PaymentRequest()
                {
                    ...
                    ObligedPersonIdent = !string.IsNullOrEmpty(request.ObligedPersonIdent) ? request.ObligedPersonIdent : obligation.ObligedPersonIdent,
                    ObligedPersonIdentType = request.ObligedPersonIdentType.HasValue ? request.ObligedPersonIdentType : obligation.ObligedPersonIdentType,
                    RegistrationDataID = registrationData.RegistrationDataID,

...
namespace EAU.Payments.Obligations.Repositories
{
    ...
    internal class ObligationRepository : EAURepositoryBase<Obligation, long?, ObligationRepositorySearchCriteria, ObligationDataContext>, IObligationRepository
    {
        ...
        protected override async Task UpdateInternalAsync(ObligationDataContext context, Obligation item, CancellationToken cancellationToken)
        {
            await context.ObligationUpdateAsync(
                item.ObligationID,
                (byte?)item.Status,
                item.Amount,
                item.DiscountAmount,

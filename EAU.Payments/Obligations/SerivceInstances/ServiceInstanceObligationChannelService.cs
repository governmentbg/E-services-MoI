    ...
    public class ServiceInstanceObligationChannelService : IObligationChannelService
    {
        ...
        public async Task<OperationResult<List<Obligation>>> SearchAsync(ObligationChannelSearchCriteria criteria, CancellationToken cancellationToken)
        {
            ...
            if (obligation != null )
            {
                ...
                {
                    return new OperationResult<List<Obligation>>("GL_EXPIRED_OBLIGATION_E", "GL_EXPIRED_OBLIGATION_E") //Срокът за извършване на плащането е изтекъл
                    {
                        Result = new List<Obligation>() { obligation }
                    };
                }
                return new OperationResult<List<Obligation>>(OperationResultTypes.SuccessfullyCompleted)
                {
                    Result = new List<Obligation>() { obligation }
                };
            }
        ...
        private async Task<Obligation> GetObligationAsync(long serviceInstanceID, string paymentInstructionURI, CancellationToken cancellationToken)
        {
            ...
            if (paymentInstruction != null)
                return InitObligation(serviceInstanceID, serviceInstance.ServiceID.Value, serviceInstance.CaseFileURI, paymentInstructionURI, paymentInstruction);
            else
                return null;
        }
        ...
        private Obligation InitObligation(long serviceInstanceID, int serviceID, string caseFileURI, string paymentInstructionURI, PaymentInstructions instruction)
        {

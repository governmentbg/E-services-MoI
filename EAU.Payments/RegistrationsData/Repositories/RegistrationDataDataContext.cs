...
namespace EAU.Payments.RegistrationsData.Repositories
{
    ...
    internal class RegistrationDataDataContext : BaseDataContext
    {
        ...
        public async Task RegistrationDataDeleteAsync(int? p_registration_data_id, CancellationToken cancellationToken)
        {
            var parameters = new DynamicParameters();

            parameters.Add("@p_registration_data_id", p_registration_data_id);

            await _dbContext.SPExecuteAsync("[pmt].[p_n_d_registration_data_delete]", parameters, cancellationToken);
        }

    ...
    internal class BTrustProcessorService : IBTrustProcessorService
    {
        ...
        public async Task<OperationResult> CreateRemoteSignRequestAsync(Guid processID, long SignerID, BtrustUserInputRequest btrustUserInput, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (btrustUserInput.InputType == BtrustUserInputTypes.PROFILE)
                {
                    ...
                    var authorizationResponse = await btrustRemoteClient.ClientAuthUsingPOSTAsync("en", authInfo, _signModuleOptions.CurrentValue.SIGN_BTRUST_REMOTE_RELYINGPARTY_ID);
                ...
                if (btrustUserInput.InputType == BtrustUserInputTypes.PROFILE)
                {
                    clientCertResponse = await btrustRemoteClient.GetCertUsingGETAsync("en", btrustUserInput.Input, _signModuleOptions.CurrentValue.SIGN_BTRUST_REMOTE_RELYINGPARTY_ID);
                }
                else
                {
                    var clientCertRes = await CheckAndGetUserCert(btrustRemoteClient, btrustUserInput.Input);
                ...
                SendSignResponse signResponse = await btrustRemoteClient.SendSignRequestUsingPOSTAsync(
                    accept_language: "en"
        ...
        public async Task<OperationResult<BtrustPullingResult>> TryCompleteRemoteSigning(Guid processID, long SignerID, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var signedContentResponse = await _btrustRemoteClientFactory.GetBtrustRemoteClient().GetSignedResultUsingGETAsync("en", currentSigner.TransactionID, _signModuleOptions.CurrentValue.SIGN_BTRUST_REMOTE_RELYINGPARTY_ID);
        ...
        private async Task<OperationResult<ClientCertResponse>> CheckAndGetUserCert(IBtrustRemoteClient btrustRemoteClient, string identifier)
        {
            try
            {
                var clientCertResponse = await btrustRemoteClient.GetCertByPersonalIdUsingGETAsync(
                    "en"
                    , identifier

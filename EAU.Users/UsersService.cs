...
namespace EAU.Users
{
    ...
    public class UsersService : IUsersService
    {
        ...
        public async Task<OperationResult<UserRegistrationResult>> BeginPublicUserRegistrationAsync(string email, string password, CancellationToken cancellationToken)
        {
        ...
        public Task<OperationResult<bool>> CancelPublicUserRegistrationAsync(Guid processGuid, CancellationToken cancellationToken)
        {
            return DbContextOperationExecutor.ExecuteAsync(async (dbContext, token) =>
            {
                ...
                if (process.Status == UserProcessStatuses.Confirmed) return ReturnErrorResult(E_REGISTRATION_PROCESS_ERROR);
        ...
        public Task<OperationResult<bool>> RegisterUserNRAAuthentication(string identifier, int currentUserId, IPAddress ipAddress, CancellationToken cancellationToken)
            => RegisterExternalUserAuthentication(identifier, currentUserId, AuthenticationTypes.NRA, ipAddress, cancellationToken);
        ...
        public Task<OperationResult<bool>> RegisterUserEAuthAuthentication(string identifier, int currentUserId, IPAddress ipAddress, CancellationToken cancellationToken)
            => RegisterExternalUserAuthentication(identifier, currentUserId, AuthenticationTypes.EAuth, ipAddress, cancellationToken);

        private Task<OperationResult<bool>> RegisterExternalUserAuthentication(string identifier, int currentUserId, AuthenticationTypes authenticationType, IPAddress ipAddress, CancellationToken cancellationToken)
        {
            return DbContextOperationExecutor.ExecuteAsync(async (dbContext, token) =>
            {
        ...
        public async Task<OperationResult<bool>> RegisterUserCertificateAuthenticationAsync(X509Certificate2 certificate, int currentUserId, IPAddress ipAddress, CancellationToken cancellationToken)
        {
